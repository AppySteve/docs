<AccordionGroup>
  <Accordion title="[MARKETING] Lincoln — SEO Specialist" icon="magnifying-glass">
  ...
  <Accordion title="[MARKETING] Marcus — Brand Strategist" icon="compass">
  ...
  <Accordion title="[MARKETING] Paige — Content Strategist" icon="pen-nib">
  ...
  <Accordion title="[MARKETING] Maven — Social Media Manager" icon="bullhorn">
  ...
  <Accordion title="[SALES] Piper — Business Development Specialist" icon="envelope">
  ...
  <Accordion title="[SALES] Scout — Competitive Analyst" icon="binoculars">
  ...
  <Accordion title="[SALES] Alexander – Account Executive" icon="briefcase">
    Alexander works the deals already in motion. Inbound leads, booked meetings, open opportunities — he qualifies them hard, keeps the pipeline honest, and carries them through close, renewal, and expansion. He doesn't generate lists or write cold outreach. He takes a real deal and figures out what it needs to advance.

    Most pipeline reviews are status updates. Alexander's are strategy sessions.

    ### What he does

    - **Qualifies rigorously.** BANT, MEDDIC, or your framework of choice. Flags deals that don't belong in the pipeline and tells you why.
    - **Preps for every meeting.** Research, discovery questions, objection anticipation, agenda — so reps show up ready, not winging it.
    - **Drafts follow-up sequences and proposals.** Tailored to the deal stage, the stakeholder, and the conversation that just happened. Never auto-sends.
    - **Runs renewals and account plans before they come due.** Surfaces at-risk signals, identifies upsell and cross-sell angles, and prepares the QBR before it's on the calendar.
    - **Keeps the forecast honest.** Period-over-period pipeline analysis, deal velocity, close rate by stage — so leadership sees what's real, not what's optimistic.

    ### What he delivers

    - Deal qualification scorecards (BANT/MEDDIC)
    - Meeting prep briefs with discovery questions and objection maps
    - Follow-up email sequences and proposal drafts
    - Account plans and renewal briefs
    - QBR decks and forecast summaries

    ### Best with

    - **CRM** – HubSpot, Salesforce (pipeline data, deal stages, contact records)
    - **Email** – Gmail, Outlook (drafting follow-up sequences for review)
    - **Calendar** – Google Calendar (meeting prep, renewal scheduling)
    - **Delivery** – Slack, Microsoft Teams

    ### How it works

    Give Alexander a deal, a meeting, or a pipeline snapshot. He reviews what you have, asks what's missing, and tells you exactly what the deal needs to move. Drafts go into your review queue — nothing sends without your sign-off. Pricing is never invented.

    ### Try him with

    - "Qualify this inbound lead against MEDDIC and tell me if it's worth pursuing."
    - "Prep me for tomorrow's discovery call with [company]."
    - "Draft a follow-up sequence for a deal that went quiet after the demo."
    - "Our [customer] renewal is in 60 days — build me an account plan."
    - "Review this quarter's pipeline and tell me where the forecast is soft."
  </Accordion>

  <Accordion title="[FINANCE] Sage — Financial Analyst" icon="chart-line">
  ...
  <Accordion title="[FINANCE] Audrey — Bookkeeper" icon="book">
  ...
  <Accordion title="[FINANCE] Sloane — Payroll Operations Manager" icon="dollar-sign">
  ...
  <Accordion title="[OPERATIONS] Sarah — Executive Assistant" icon="calendar">
  ...
  <Accordion title="[OPERATIONS] Dexter — Business Operations Analyst" icon="chart-column">
  ...
  <Accordion title="[OPERATIONS] Joy — Customer Service Manager" icon="comment">
  ...
  <Accordion title="[CREATIVE] Cole – Videographer" icon="video">
    Cole generates and edits actual video assets. Give him a brief, a script, or raw footage — he builds the finished clip. Short-form social content, product demos, animated explainers, ad creative, motion graphics — Cole handles the full production job, not just the generation step.

    Most AI video tools stop at the clip. Cole finishes the job.

    ### What he does

    - **Generates video from a prompt or still image.** Short clips created from text descriptions or animated from an existing image — useful for social content, explainers, and ad creative when you don't have footage.
    - **Edits and assembles footage.** Trim, splice, cut to rhythm, layer B-roll — takes raw or generated clips and builds a finished piece.
    - **Formats for platform.** Aspect ratio conversions (16:9, 9:16, 1:1), duration cuts per platform spec, compression for upload.
    - **Adds captions and subtitles.** Auto-generated and timed to the audio track.
    - **Sourced music and overlays.** Background audio, lower thirds, motion graphic elements where the brief calls for them.

    ### What he delivers

    - Generated video clips (from prompt or still image)
    - Edited and assembled video files (MP4)
    - Platform-formatted versions (Reels, TikTok, YouTube, LinkedIn)
    - Captioned/subtitled cuts
    - Product demos and animated explainers

    ### Best with

    - **Uploaded footage** – provide raw clips directly
    - **Script or brief** – the tighter the brief, the better the output
    - **Delivery** – Slack, email (for sharing finished files)

    ### How it works

    Brief Cole on what you're making — platform, length, tone, any footage you have — and he takes it from there. For generation, a text prompt or still image is enough. For editing, upload your footage and describe the cut. Output is a finished video file ready to publish.

    ### Try him with

    - "Create a 15-second product demo clip for Instagram Reels."
    - "Animate this product screenshot into a short explainer video."
    - "Edit this raw interview footage down to a 60-second highlight cut."
    - "Add captions to this video and reformat it for TikTok (9:16)."
    - "Make a short ad video for our new feature launch."
  </Accordion>

  <Accordion title="[CREATIVE] Hamilton – Website Designer" icon="globe">
    Hamilton builds complete static websites end to end — brief in, live URL out. He owns the whole job: HTML structure, CSS system, copy when the brief is thin, images when the design calls for them, favicon, OG image, meta tags, and deploy. You don't manage a handoff between a designer and a developer. You just get a live site.

    Most website tools produce templates. Hamilton produces sites.

    ### What he does

    - **Designs and builds from scratch.** Semantic HTML, separate CSS files, mobile-first responsive layouts, modern CSS (grid, flexbox, custom properties). No page builders, no bloat.
    - **Handles copy when needed.** If the brief is thin, he writes the headlines, body copy, and CTAs — then you refine.
    - **Sources and integrates imagery.** Stock photography, AI-generated visuals, uploaded assets — placed and styled to the design system.
    - **Takes care of the details.** Favicon set, Open Graph image, meta title and description, proper heading hierarchy, and load performance.
    - **Deploys to a live URL.** Publishes the finished site to a public link you can share immediately.
    - **Pulls in libraries when they earn it.** Chart.js for data, Alpine.js for interactivity, HTMX for dynamic behavior — CDN-loaded, only when the design calls for them.

    ### What he delivers

    - Complete static websites (HTML/CSS)
    - Landing pages, microsites, one-pagers
    - Event and RSVP pages, portfolio sites, marketing pages
    - Lead-gen pages, product pages, link-in-bio pages
    - Live public URL on completion

    ### Best with

    - **Brand assets** – logos, color palette, fonts (speeds up the design pass)
    - **Copy** – headlines, body text (use what you have; Hamilton fills the gaps)
    - **Delivery** – Slack, email (for sharing the live link)

    ### How it works

    Give Hamilton the brief — type of site, purpose, audience, any assets you have. He designs, builds, and deploys. The live URL comes back when it's done. Changes go back to Hamilton as follow-up requests.

    ### Try him with

    - "Build a landing page for our new product launch — here's the brief."
    - "Create a one-page event site with an RSVP form for our webinar."
    - "Make a simple portfolio site for our agency."
    - "Build a lead-gen page for our free trial offer."
    - "I need a link-in-bio page for our Instagram — clean, minimal, on-brand."
  </Accordion>

  <Accordion title="[CREATIVE] Margot – Visual Designer" icon="palette">
    Margot builds brand-consistent visual assets end to end. She starts by extracting a brand profile — from a logo, website, or brand guide — and uses it to lock palette, typography, lighting, and composition across every asset she creates. Single pieces or coherent packs, generated from scratch or edited from what you have.

    Most AI image tools produce output. Margot produces assets that fit your brand.

    ### What she does

    - **Extracts and maintains a brand profile.** Pulls the visual system from whatever source material you have — logo, site, existing deck — and holds it consistent across every asset in the project.
    - **Generates brand-aligned visuals.** Hero images, marketing graphics, social headers, posters, illustrations, icons — designed to the extracted brand, not generic stock.
    - **Produces logo and identity assets.** Logo variations, favicon, OG image — the foundational pieces a brand needs.
    - **Edits existing images mechanically.** Resize, format conversion, compression, watermarking, print prep (DPI, bleed, CMYK guidance) — deterministic operations on files you already have.
    - **Builds asset packs.** Coherent sets for a campaign, a product launch, or a social channel — consistent across every piece.

    ### What she delivers

    - Brand profile (extracted palette, typography, style rules)
    - Logo variations, favicon, OG image
    - Hero images and marketing graphics
    - Social headers, feed graphics, poster templates
    - Icon sets and illustrations
    - Resized, converted, compressed, or watermarked image files

    ### Best with

    - **Existing brand assets** – logo files, brand guide, website URL (for brand extraction)
    - **Reference images** – examples of the style you want
    - **Delivery** – Slack, email, Google Drive, OneDrive

    ### How it works

    Give Margot a brief and any brand assets you have. She extracts your visual system first (or confirms what you've described), then produces assets to that spec. For mechanical edits, upload the file and describe the operation. She flags where source material needs to come from you rather than inventing it.

    ### Try her with

    - "Create a hero image for our product launch page — here's our brand guide."
    - "Build a social header pack for LinkedIn and Twitter."
    - "Extract our brand palette from our website and produce an icon set."
    - "Resize these 10 images for web and compress them under 200KB."
    - "Design a favicon and OG image that match our logo."
  </Accordion>
</AccordionGroup>
