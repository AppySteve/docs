...
Your agent uses Google News Search to find the latest election coverage, breaking news articles, and recent developments from multiple news sources. It then synthesizes this real-time news information into a comprehensive brief for your client - giving them insights based on the very latest news coverage, not outdated information.
  ...
  <Accordion title="Image Generation" icon="globe-pointer">
    ### What it does
...
.
  ...
  <Accordion title="Type2Thinker Tool" icon="hammer">
    ### What it does

   Type2Thinker creates code (Typescript), evaluates that code and uses it on behalf of your agent.

    ### Why you'd use it

    Type 2 thinking, also known as System 2 thinking refers to deliberate, analytical, and effortful thinking. It contrasts with System 1, which is automatic and unconscious. Many LLM models, by default, operate more in a System 1 mode. Utilize Type 2 to force a more reasoned approach, or to utilize code when that is the most effective way to get things done.

    ### Suggested use cases

    Here are a few of the most common scenarios where Type2 Thinker might come in handy:

    - **Large data sets** - Extract information and analyze large files without hitting context limits of LLMs
    - **Complex calculations** - Perform computations that standard LLMs struggle with
