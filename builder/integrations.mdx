...
This means you can create powerful, personalized experiences that actually integrate with your users' existing workflows, all without touching a single line of integration code.

<Note>
  If you are looking for an integration not already available, contact support. Appy has hundreds of integrations ready to enable when a builder is ready to use them.
</Note>
...
Depending on the integration, connections use secure OAuth authentication or secretely store an API key provider by the users- the AI never sees or stores your subscribers' passwords. Each integration only gets access to the specific actions you've configured, and subscribers can revoke access anytime.
